...
def page():
    ...
    st.write("-----")
    st.subheader(f"**1. :green[AI습득 정도 선택] : :orange[{ai_level}]**")
    st.subheader(f"**2. :green[신분 선택] : :orange[{activity_field}]**")
    st.subheader(f"**3. :green[목적 선택] : :orange[{selected_purposes_str}]**")
