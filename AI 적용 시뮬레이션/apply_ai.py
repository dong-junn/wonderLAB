...
def page():
    ...
    clicked_data_collection_page = st.checkbox(" '데이터 수집' 페이지를 클릭했습니다.", key="clicked_data_collection_page")
    uploaded_file = st.checkbox(" '파일찾아보기' 버튼을 눌러 저장한 파일을 업로드했습니다.", key="uploaded_file")
    checked_uploaded_dataset = st.checkbox(" 페이지 하단에서 업로드된 데이터셋이 정상적으로 보이는지 확인했습니다.", key="checked_uploaded_dataset")
    ...
    clicked_data_analysis = st.checkbox(" '데이터 분석' 버튼을 클릭했습니다.", key="clicked_data_analysis")
    selected_target_variable = st.checkbox(" '추가 탐사 준비' 섹션에서 관심 있는 열을 선택하여 Target 변수를 지정했습니다.", key="selected_target_variable")
    checked_correlation_matrix = st.checkbox(" 타겟 변수와 다른 변수들 간의 상관관계 매트릭스를 확인했습니다.", key="checked_correlation_matrix")
    ...
    clicked_model_building = st.checkbox(" '모델 빌딩' 버튼을 클릭했습니다.", key="clicked_model_building")
    selected_target_column = st.checkbox(" '타겟 컬럼'에서 예측하고자 하는 타겟 변수를 선택했습니다.", key="selected_target_column")
    clicked_start_model_creation = st.checkbox(" '마술을 해 보셔요' 버튼을 클릭하여 모델 생성을 시작했습니다.", key="clicked_start_model_creation")
    waited_for_model_creation = st.checkbox(" 모델 생성이 완료될 때까지 30초에서 1분 정도 기다렸습니다.", key="waited_for_model_creation")
    ...
    waited_for_model_creation_btn = st.checkbox(" '모델 테스트'라는 페이지 버튼을 클릭했습니다.", key="waited_for_model_creation_btn")
    ...
    found_prediction_method_section = st.checkbox(" 왼쪽 사이드 메뉴에서 '어떻게 예측하시겠습니까?' 섹션을 찾았습니다.", key="found_prediction_method_section")
    selected_online_option = st.checkbox(" 나타나는 선택사항 중에서 '온라인'을 선택했습니다.", key="selected_online_option")
    ...
    prepared_all_input_values = st.checkbox("오른쪽에 나타난 입력창에서, 인공지능 모델에 적용할 모든 입력값을 준비했습니다.", key="prepared_all_input_values")
    filled_all_input_values = st.checkbox("가상의 웹앱에 모든 필요한 입력값을 채웠습니다.", key="filled_all_input_values")
    ...
    clicked_predict_button = st.checkbox(" '예측하다' 버튼을 클릭하여 모델 테스트를 진행했습니다.", key="clicked_predict_button")
    successful_test_result = st.checkbox(" 테스트 결과가 성공적으로 산출되었습니다.", key="successful_test_result")
    ...
    saved_test_result_screenshot = st.checkbox(" '테스트 결과 장면'을 스크린캡쳐하여 개인 PC에 저장했습니다.", key="saved_test_result_screenshot")
    ...
    appl_stp1 = check_completion(clicked_data_collection_page, uploaded_file, checked_uploaded_dataset, clicked_data_analysis, selected_target_variable, checked_correlation_matrix, difficulty= difficulty_checked_uploaded_dataset or difficulty_checked_correlation_matrix)
    st.subheader(f"단계1: {appl_stp1}")

    appl_stp2 = check_completion(clicked_model_building, selected_target_column, clicked_start_model_creation, waited_for_model_creation, difficulty= difficulty_waited_for_model_creation)
    st.subheader(f"단계2: {appl_stp2}")

    appl_stp3 = check_completion(waited_for_model_creation_btn, difficulty= difficulty_clicked_model_test_page)
    st.subheader(f"단계3: {appl_stp3}")

    appl_stp4 = check_completion(found_prediction_method_section, selected_online_option, difficulty= difficulty_selected_online_option)
    st.subheader(f"단계4: {appl_stp4}")

    appl_stp5 = check_completion(prepared_all_input_values, filled_all_input_values, difficulty= difficulty_filled_all_input_values)
    st.subheader(f"단계5: {appl_stp5}")

    appl_stp6 = check_completion(clicked_predict_button, successful_test_result, difficulty= difficulty_successful_test_result)
    st.subheader(f"단계6: {appl_stp6}")

    appl_stp7 = check_completion(saved_test_result_screenshot, difficulty= difficulty_saved_test_result_screenshot)
    st.subheader(f"단계7: {appl_stp7}")