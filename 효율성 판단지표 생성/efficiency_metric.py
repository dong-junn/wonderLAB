...
def page():
    ...
    col1, col2 = st.columns(2)
    with col1:
        time_before = st.text_input("적용 전 시간", key='time_before')
    with col2:
        time_after = st.text_input("적용 후 시간", key='time_after')

    # 3. 비용 절감 효과 측정
    st.subheader("3. AI 솔루션 도입에 따른 비용 절감 효과 측정")
    col1, col2 = st.columns(2)
    with col1:
        cost_before = st.text_input("도입 전 비용", key='cost_before')
    with col2:
        cost_after = st.text_input("도입 후 비용", key='cost_after')
    ...
    st.subheader("5. AI 솔루션 도입이 업무 결과물의 품질에 미치는 영향")
    ...
    st.session_state['quality_impact'] = st.selectbox(
        "품질 영향 선택",
        ["상당히 향상", "약간 향상", "변화 없음", "저하"],
        index=["상당히 향상", "약간 향상", "변화 없음", "저하"].index(st.session_state['quality_impact']),
        on_change=update_quality_impact,
        args=(st.session_state['quality_impact'],)
    )

    # 질문 5: 업무 결과물의 품질 변화 측정 지표
    st.subheader("6. 업무 결과물의 품질 변화 측정 지표")
    ...
    st.session_state['quality_measurement'] = st.selectbox(
        "품질 변화 측정 지표 선택",
        ["고객 만족도", "오류율 감소", "처리 속도"],
        index=["고객 만족도", "오류율 감소", "처리 속도"].index(st.session_state['quality_measurement']),
        on_change=update_quality_measurement,
        args=(st.session_state['quality_measurement'],)
    )

    # 질문 6: AI 도입 전후 업무에 필요한 인력 수 변화
    st.subheader("7. AI 도입 전후 업무에 필요한 인력 수 변화")
    ...
    st.subheader("9. AI 솔루션 도입이 업무 절차에 미치는 영향")
    ...
    st.subheader("10. 업무 절차의 간소화 평가 기준")
